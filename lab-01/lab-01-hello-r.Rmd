...
One goal of this lab is to introduce you to R, RStudio, and RMarkdown files, which we will be using throughout the course both to learn the statistical concepts discussed in the course and to analyze real data and come to informed conclusions. 
...
-   In RStudio Cloud, click on your name on the top right corner to open the right menu.
...
-   In the left sidebar, click *Developer settings*, then select *Personal access tokens*
...
-   Now go back to Posit Cloud and open any project in our ENST 222 workspace. Copy and paste the code below into your R console and replace the `---` with your PAT. Make sure to keep the quotes. Hit enter to run the code. You might be prompted to enter your PAT again. 
```{r}
credentials::set_github_pat("---")
```

-   You should now be all set up with GitHub!
...
Next, open the Markdown Quick Reference, accessible via the Help menu. Use markdown syntax to do the following anywhere in the last paragraph of random text in the starter .Rmd document:
...
✅ ⬆️ *You're done! Commit all remaining changes, use the commit message "Done with lab 1!", and push. Make sure to commit and push all changed files so that your Git pane is clear. Then make sure all of the documents are updated in your GitHub repo.*
